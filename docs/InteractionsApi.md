...
namespace Example
{
    public class CreateGuestDonationExample
    {
        public static void Main()
        {
            Configuration config = new Configuration();
            ...
            HttpClientHandler httpClientHandler = new HttpClientHandler();
            var apiInstance = new InteractionsApi(httpClient, config, httpClientHandler);
            var wTGuestDonationRequest = new WTGuestDonationRequest(); // WTGuestDonationRequest | 

            try
            {
                // Create a guest tip / donation PaymentIntent (KAN-861)
                WTGuestCreatePaymentIntentResponse result = apiInstance.CreateGuestDonation(wTGuestDonationRequest);
                Debug.WriteLine(result);
            }
            catch (ApiException  e)
            {
                Debug.Print("Exception when calling InteractionsApi.CreateGuestDonation: " + e.Message);
                Debug.Print("Status Code: " + e.ErrorCode);
                Debug.Print(e.StackTrace);
            }
        }
...
#### Using the CreateGuestDonationWithHttpInfo variant
This returns an ApiResponse object which contains the response data, status code and headers.
...
{
    // Create a guest tip / donation PaymentIntent (KAN-861)
    ApiResponse<WTGuestCreatePaymentIntentResponse> response = apiInstance.CreateGuestDonationWithHttpInfo(wTGuestDonationRequest);
    Debug.Write("Status Code: " + response.StatusCode);
    Debug.Write("Response Headers: " + response.Headers);
    Debug.Write("Response Body: " + response.Data);
}
...
{
    Debug.Print("Exception when calling InteractionsApi.CreateGuestDonationWithHttpInfo: " + e.Message);
    Debug.Print("Status Code: " + e.ErrorCode);
    Debug.Print(e.StackTrace);
}
```
...
| Name | Type | Description | Notes |
|------|------|-------------|-------|
| **wTGuestDonationRequest** | [**WTGuestDonationRequest**](WTGuestDonationRequest.md) |  |  |
