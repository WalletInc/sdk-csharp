...
<a id="updatepagetipspreference"></a>
# **UpdatePageTipsPreference**
> Employee UpdatePageTipsPreference (UpdatePageTipsPreferenceRequest updatePageTipsPreferenceRequest)
...
namespace Example
{
    public class UpdatePageTipsPreferenceExample
    {
        public static void Main()
        {
            Configuration config = new Configuration();
            ...
            HttpClientHandler httpClientHandler = new HttpClientHandler();
            var apiInstance = new EmployeesApi(httpClient, config, httpClientHandler);
            var updatePageTipsPreferenceRequest = new UpdatePageTipsPreferenceRequest(); // UpdatePageTipsPreferenceRequest | 

            try
            {
                // Turns the admin-portal page tips bar on or off for this employee, synced across devices (KAN-963)
                Employee result = apiInstance.UpdatePageTipsPreference(updatePageTipsPreferenceRequest);
                Debug.WriteLine(result);
            }
            catch (ApiException  e)
            {
                Debug.Print("Exception when calling EmployeesApi.UpdatePageTipsPreference: " + e.Message);
                Debug.Print("Status Code: " + e.ErrorCode);
                Debug.Print(e.StackTrace);
            }
        }
...
#### Using the UpdatePageTipsPreferenceWithHttpInfo variant
This returns an ApiResponse object which contains the response data, status code and headers.
...
{
    // Turns the admin-portal page tips bar on or off for this employee, synced across devices (KAN-963)
    ApiResponse<Employee> response = apiInstance.UpdatePageTipsPreferenceWithHttpInfo(updatePageTipsPreferenceRequest);
    Debug.Write("Status Code: " + response.StatusCode);
    Debug.Write("Response Headers: " + response.Headers);
    Debug.Write("Response Body: " + response.Data);
}
...
{
    Debug.Print("Exception when calling EmployeesApi.UpdatePageTipsPreferenceWithHttpInfo: " + e.Message);
    Debug.Print("Status Code: " + e.ErrorCode);
    Debug.Print(e.StackTrace);
}
```
...
| Name | Type | Description | Notes |
|------|------|-------------|-------|
| **updatePageTipsPreferenceRequest** | [**UpdatePageTipsPreferenceRequest**](UpdatePageTipsPreferenceRequest.md) |  |  |
